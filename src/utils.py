...
from src.env_utils import FrameStack, Environment, Minigrid2Image, ProcGenEnvironment, Procgen2RGB
...
def create_buffers_clride(obs_shape, num_actions, flags) -> Buffers:
    T = flags.unroll_length
    specs = dict(
        frame=dict(size=(T + 1, *obs_shape), dtype=torch.uint8),
        reward=dict(size=(T + 1,), dtype=torch.float32),
        ...
        episode_state_count=dict(size=(T + 1,), dtype=torch.float32),
        train_state_count=dict(size=(T + 1,), dtype=torch.float32),
        generator_logits=dict(size=(T + 1,), dtype=torch.uint8),
...
def act(i: int, free_queue: mp.SimpleQueue, full_queue: mp.SimpleQueue,
    ...
    try:
        ...
        if 'procgen' in flags.env:
            env = ProcGenEnvironment(gym_env)
        else:
...
def act_clride(i: int, free_queue: mp.SimpleQueue, full_queue: mp.SimpleQueue,
        model: torch.nn.Module, generator_model: torch.nn.Module, buffers: Buffers,
        episode_state_count_dict: dict, train_state_count_dict: dict,
        initial_agent_state_buffers, flags):
    try:
        log.info('Actor %i started.', i)
